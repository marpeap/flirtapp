...
export function getRandomCupidAvatarPath() {
  ...
  const images = CUPID_CATEGORIES[randomCategory];
  const randomNum = images[Math.floor(Math.random() * images.length)];
  return `/cupids/${randomCategory}/${String(randomNum).padStart(3, '0')}.png`;
}
 ...
 * @returns {string} - Le chemin de l'avatar (ex: "/cupids/Ghostz/138.png")
 */
export function getCupidAvatarByCategory(category) {
  ...
  return `/cupids/${category}/${imageNumber}.png`;
}
...
export function getCupidAvatarsByCategory(category) {
  ...
  return images.map((num) => `/cupids/${category}/${num}.png`);
}
...
export function getRandomCupidAvatars(count = 5) {
  ...
  for (const [category, images] of Object.entries(CUPID_CATEGORIES)) {
    for (const num of images) {
      allAvatars.push(`/cupids/${category}/${String(num).padStart(3, '0')}.png`);
    }
  }
